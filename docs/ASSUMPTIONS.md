...
The brief itself says: *"expect to make and state your own assumptions"*, this document is where we honour that.
...
- Consequence: a spot at ``x0 = 0.0`` sits exactly on a pixel centre; a spot
  at ``x0 = 0.5`` sits exactly on a pixel boundary. Both are valid inputs, there's no special-casing needed, which is part of why this convention was
  chosen.
...
- Why: dark current is the same Poisson mechanism as photon noise (random
  ...
  derivation for a tight pooled-variance standard error applies unchanged, no need to re-derive it. Note this combination is chosen for a clean,
...
- Why this matters more than a simple flux-calibration error: a uniform
  ...
  position-dependent bias in the same family as pixel locking (`psf.py`), and, being a fixed pattern rather than fresh per-frame noise, it does
...
- Why the approximation is valid here: it depends on the signal already
  having some other noise mixed in before rounding (which it does, everything upstream: photon, read, dark), so the rounding error's
...
- For an axis-aligned gradient (angle = 0 or pi/2), `gradient_frac` gives
  exactly that fraction of `mean_level` peak-to-peak, edge to edge, verified directly in `test_axis_aligned_gradient_peak_to_peak_matches_gradient_frac`.
...
- Verified `peak_pixel_fraction` behaves sensibly: it strictly decreases as
  the spot widens (`test_peak_pixel_fraction_decreases_as_spot_widens`), a wider spot spreads the same total flux more thinly.
...
- Why: this is the order the physics actually requires, not an arbitrary
  ...
  pathway, so it can only apply to light that passes through that pathway, the spot and background. Dark current and hot-pixel electrons are
...
`Simulator` defaults every noise parameter to a modest, non-zero value, nothing defaults to "off."
...
- In DN terms: at 10 electrons/DN, that's `32.9 / 10 ≈ 3.3` DN of noise, fine enough resolution relative to the 105 DN baseline for a clean
...
- Bug 2, the trial step's chi² used a different variance model than
  ...
  even when the proposed step shrank to exactly zero under heavy damping, an unwinnable comparison that rejected every single iteration, all the
...
- What actually surfaced both bugs: not a unit test, but the full SNR
  sweep experiment producing a result that was qualitatively impossible, `centroid_eff` and `fit_eff` identical to 2 decimal places at every one
...
Log-parabola interpolation is the default, not the plain parabola, verified as EXACT for noiseless Gaussian samples, not just theoretically
motivated.
...
- Verified two ways, not just derived: (1) `test_log_parabola_offset_is_exact_for_noiseless_gaussian_samples`
  ...
  log-parabola error ~8e-7 px against plain-parabola error ~0.0075 px, about 10,000x worse for the plain version, on identical data.
...
Why these are reported as Python wall-clock numbers, not a
...
is a checkable consistency result:
...
A rejected approach: summing energy across the leaked lobe
overestimates amplitude. Because the disturbance frequency
...
off-bin. This is a direct instance of the project's standing rule, verify a claim numerically before trusting it, catching a genuinely
...
10-trials-per-level Monte Carlo sweep, not a single lucky/unlucky run, an earlier single-realization prototype was explicitly discarded once the
...
own precision (~0.007-0.011 px) is negligible next to jitter (0.15 px), why the easy scenario's recovered-vs-ground-truth detection results came
...
threshold has a blind spot. Separately (SNR=50, easy amplitude 0.3 px, deliberately the easy scenario's own amplitude, to isolate this as a
...
much wider (non-diffraction-limited) profile can still have a lower peak, checked directly: the first prototype used a clutter flux that
...
reading tells the controller "still too bright" but not "by how much", the magnitude information needed for a confident one-step correction is
...
A wording correction. The first draft of the
...
A contradiction found in review. The first draft of
...
## Panel feedback and what changed

Feedback received from the interview panel after review of the earlier
`spotlab` submission, and what was done about each point.

Writing style. The documents read as machine-written. Em dashes, bold and
italic markup were removed from all five markdown files, and
self-congratulatory phrasing was reworded. The specific instruction was
to be technical and professional and to avoid narrating the integrity of
the process. Python docstrings were already free of em dashes since they
use double hyphens.

A factual correction. The `spotlab` report stated that "sub-pixel is an
aspiration, not a requirement". The brief states sub-pixel precision as a
requirement, so that claim was wrong. It does not appear in this project
...
including choices not made. That is `docs/DESIGN_RATIONALE.md`. Two
questions were named directly, why no Kalman filter and why Gauss-Newton,
and both are answered there with measurements rather than argument.

Parameter sourcing. Two previously judgement-based numbers were
researched and are now cited. The 20 Hz disturbance corresponds to
1200 RPM, inside the published 15 to 40 Hz reaction-wheel fundamental
band. The scintillation parameters give indices of 0.174 and 0.433,
inside the measured FSO range of 0.083 to 0.71 and below the 0.75
log-normal validity limit. Jitter magnitude remains an engineering
assumption because converting published microradian figures to pixels
needs a plate scale this project does not have.
